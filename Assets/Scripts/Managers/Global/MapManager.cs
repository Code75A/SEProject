...
public class MapManager : MonoBehaviour
{
    ...
    public void GenerateMapTiles(){
        // 先确保Tilemap有必要的碰撞组件
        var tilemapCollider = landTilemap.gameObject.GetComponent<TilemapCollider2D>();
        if(tilemapCollider == null) {
            tilemapCollider = landTilemap.gameObject.AddComponent<TilemapCollider2D>();
            tilemapCollider.usedByComposite = true;
        }

        var compositeCollider = landTilemap.gameObject.GetComponent<CompositeCollider2D>();
        if(compositeCollider == null) {
            landTilemap.gameObject.AddComponent<CompositeCollider2D>();
            var rb = landTilemap.gameObject.GetComponent<Rigidbody2D>();
            if(rb == null) {
                rb = landTilemap.gameObject.AddComponent<Rigidbody2D>();
    ...
    void Awake(){
        if(Instance == null) {
            ...
            // 确保Tilemap有必要的组件
            if(landTilemap != null) {
                if(landTilemap.GetComponent<TilemapCollider2D>() == null) {
                    landTilemap.gameObject.AddComponent<TilemapCollider2D>();
                }
                if(landTilemap.GetComponent<Rigidbody2D>() == null) {
                    var rb = landTilemap.gameObject.AddComponent<Rigidbody2D>();
                    rb.bodyType = RigidbodyType2D.Static;
                }
            }
        }
